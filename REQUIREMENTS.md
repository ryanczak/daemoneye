...
- **FR-1.2.2**: When activated, the application MUST capture terminal context (visible output, backscroll history, environment state) from the currently active tmux pane and provide it to the AI agent. The captured context MUST include, for each pane: (a) the shell's current working directory (`#{pane_current_path}`); (b) the OSC terminal title (`#{pane_title}`) when set by a running application (e.g., vim, ssh, k9s) and distinct from the command name; (c) a `[dead: N]` annotation for background panes whose foreground process has exited with code N (remain-on-exit mode). Additionally, the application MUST fetch high-signal tmux session environment variables via `tmux show-environment` filtered against an allowlist (AWS_PROFILE, AWS_DEFAULT_REGION, AWS_REGION, KUBECONFIG, KUBE_CONTEXT, KUBECTL_CONTEXT, VAULT_ADDR, DOCKER_HOST, DOCKER_CONTEXT, ENVIRONMENT, APP_ENV, NODE_ENV, RAILS_ENV, RACK_ENV, VIRTUAL_ENV, CONDA_DEFAULT_ENV, GOPATH, GOENV, JAVA_HOME, LANG, LC_ALL) and prepend them as a `[SESSION ENVIRONMENT]` block to every AI context snapshot.
...
- **FR-1.2.7**: The application MUST support two distinct command execution modes and the AI MUST be instructed to choose between them appropriately:
  ...
  - *Foreground mode*: Command is injected into the user's active tmux pane via `send-keys`. Completion MUST be detected cleanly by attaching a temporary `pane-title-changed` tmux hook to the target pane, keeping the terminal clean while using an event-driven IPC notification (with a slow polling fallback for incompatible shells). The user can see and interact with the command as it runs. If the user's pane is SSH'd to a remote host, the command executes on that remote host. If the AI does not explicitly set a target pane, the CLI client MUST perform automatic pane discovery before connecting: auto-select the sole sibling pane in the same window, present a numbered picker when multiple siblings exist, or offer to split the window side-by-side (default) or pick from other session panes when the chat pane is alone. The chosen pane MUST be persisted to `~/.daemoneye/pane_prefs.json` keyed by session name so the user is never prompted again after the first run. The persisted preference is always overridable by explicit AI instructions.

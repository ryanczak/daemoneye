...
- **FR-1.2.7**: The application MUST support two distinct command execution modes and the AI MUST be instructed to choose between them appropriately:
  ...
  - *Foreground mode*: Command is injected into the user's active tmux pane via `send-keys`, with `; <binary> notify activity <pane_id> <hook_id> <session>` appended so that completion is detected instantly via an event-driven IPC hook rather than polling. The user can see and interact with the command as it runs. If the user's pane is SSH'd to a remote host, the command executes on that remote host.
...
- **FR-1.2.13**: The daemon MUST support an overarching Information Window (`de-info`). The daemon MUST ensure this window exists in the session upon startup, structured as a 3-pane layout containing tails of `daemon.log`, `activity.log`, and `events.jsonl`.

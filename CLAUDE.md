...
| `src/memory/index.rs` | **Stub.** `fts5_search()` returns an empty `Vec` and the BM25 scoring is not yet wired. Its only caller is `memory_prompt::ftsearch_memories()`, which therefore always finds nothing. Real memory search is the grep scan in `src/search.rs`. Un-stubbing is future work (see `docs/architecture.md` § 5) |

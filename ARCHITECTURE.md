...
- **AI Agent Engine** (`daemon/server.rs` & `daemon/executor.rs`): Orchestrates the full request lifecycle. `server.rs` handles the IPC connection loop, loads session history, builds the prompt (host context + execution context + terminal snapshot), and streams AI events. `executor.rs` processes the resulting tool calls, handling user approval gates and dispatching background/foreground execution.

...
- **Dual Command Execution** (`daemon.rs`):
  - *Background mode* (`background=true`): Runs in a dedicated tmux window (`de-bg-<id_short>`) on the daemon host. The command is sent via `tmux send-keys` and appended with `; echo '__DE_EXIT__'$?`. `capture_until_sentinel()` polls `capture-pane` every 200 ms until the sentinel appears (up to 300 s). The exit code is extracted from the sentinel line and the window is always killed after output is captured. This gives the command access to the user's full shell environment (PATH, aliases, functions). If the command contains `sudo`, the daemon first sends a `CredentialPrompt` IPC response; the client reads the password with echo disabled and returns it via `CredentialResponse`; the credential is injected into the window via `send-keys` after detecting the sudo password prompt in `capture-pane` output. The credential is never logged or transmitted to the AI. When `capture_until_sentinel()` times out without finding the sentinel (e.g., the process was killed or the window died), the daemon falls back to `pane_dead_status()` to retrieve the actual exit code rather than silently reporting success; exit code 124 is used when the pane status is also unavailable (POSIX timeout convention).

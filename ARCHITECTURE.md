...
- **Dual Command Execution** (`daemon.rs`):
  ...
  - *Foreground mode* (`background=false`): Injects `<cmd>; tmux wait-for -S de_<id>` into the user's active tmux pane via `tmux send-keys`. The daemon simultaneously blocks on `tmux wait-for de_<id>` in a spawned tokio task; the task unblocks the instant the shell executes the trailing signal suffix — zero-latency completion detection with no stability polling. A lightweight 50 ms fallback loop checks `pane_current_command == idle_cmd` in parallel to handle Ctrl+C (where the user interrupts before the signal suffix can run); on fallback, the daemon signals the channel itself (`tmux wait-for -S de_<id>`) so the subprocess exits cleanly. For sudo commands, the daemon polls `pane_current_command` at 100 ms intervals (up to 3 s, with early exit if the pane returns to idle, i.e. NOPASSWD/cached credentials) to detect whether a password prompt appears; if one is found, a `SystemMsg` notification is sent and the working pane is focused so the user can type their password. Full command completion is still signalled by the `tmux wait-for` mechanism regardless of the sudo path taken.
...
8. On approval:
   ...
   - *Foreground*: The daemon injects `<cmd>; tmux wait-for -S de_<id>` into the user's pane and blocks on `tmux wait-for de_<id>` (30-second overall timeout) until the shell signals completion. If `sudo` is detected during a brief 3 s polling window, a `SystemMsg` switches focus to the working pane for password entry; NOPASSWD/cached-credential runs exit this window immediately. After the wait-for unblocks, 200 lines are captured from the pane.

...
4. On the **first turn**, the daemon captures the last 200 lines from the client's pane via `get_labeled_context()`, runs the sensitive-data filter, and fetches the cached system context (or collects it fresh on the first-ever request). Non-active panes are classified into `[VISIBLE PANE]` (same window as chat), `[BACKGROUND PANE]` (daemon-launched `de-bg-*`/`de-sched-*`), and `[SESSION PANE]` (other user windows), each with working directory, OSC terminal title, synchronized-input flag, `[dead: N]` where applicable, and an activity annotation (`[active Xs ago]` / `[idle Nm]` / `[idle NhNm]`) based on the last recorded keypress or output in that pane. The active pane content has ANSI SGR colour codes converted to semantic markers (`[ERROR: …]` / `[WARN: …]` / `[OK: …]`) by `annotate_ansi()` (R2), so the AI can identify failure and success lines without parsing raw escape codes. High-signal tmux session environment variables are prepended as a `[SESSION ENVIRONMENT]` section. When two or more tmux sessions exist, an `[OTHER SESSIONS]` block is appended listing each non-current session's name, window count, last-activity age, and attachment state. On **subsequent turns** the terminal snapshot is omitted from the user message — the AI calls `get_terminal_context` when it needs a fresh view of the screen.

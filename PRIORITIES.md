...
- `config.rs`: add `environment: String` (default `"personal"`) to `AiConfig` or a new
  `[context]` section.
- `daemon/session.rs` / `sys_context.rs`: prepend `Environment: <value>` to the host-context block.
...
- `daemon/server.rs`: when iterating cached pane summaries, prefix the source pane's block with

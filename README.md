...
- **Collaborative Execution (Tool Calling)** — The AI can propose commands to fix issues. Each tool call presents a three-option prompt: `[y]es` (approve once), `[a]pprove session` (auto-approve all commands of this class for the rest of the session), or `[N]o`. Two independent approval classes exist — *regular* and *sudo* — so sudo commands always prompt separately until explicitly session-approved. Two execution modes: *background* (daemon subprocess, output summarized in chat and logged) and *foreground* (injected into your tmux active pane via `send-keys`, visible and interactive; the AI receives the output the moment the command finishes). Foreground commands appear in your pane with a `; tmux wait-for -S ...` suffix appended — this is how the daemon detects completion instantly rather than polling.

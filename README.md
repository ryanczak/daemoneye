...
- **Collaborative Execution (Tool Calling)** — The AI can propose commands to fix issues. Each tool call presents a three-option prompt: `[y]es` (approve once), `[a]pprove session` (auto-approve all commands of this class for the rest of the session), or `[N]o`. Two independent approval classes exist — *regular* and *sudo* — so sudo commands always prompt separately until explicitly session-approved. Two execution modes: *background* (runs in a dedicated tmux window `de-bg-<session_name>-...` on the daemon host using the user's configured shell; output is captured and saved to `~/.daemoneye/pane_logs/` before the window is automatically garbage collected) and *foreground* (command is injected into your active tmux pane via `send-keys` with a `daemoneye notify activity` suffix appended so completion is detected instantly via an event-driven hook rather than polling; the output is visible and interactive in your pane).

...
- **AI-Powered Capabilities**:
  ...
  - **Dual Execution Modes**: The AI chooses between two command execution modes. *Background mode* runs the command in a dedicated tmux window (`de-bg-*`) on the daemon host using the user's configured shell — capturing output via `capture-pane` and returning it to the AI; the window is always cleaned up after the result is captured, with a `tmux display-message` overlay showing `` `cmd` succeeded/failed in pane <id> `` when complete. *Foreground mode* appends `; daemoneye notify activity` to the command before injecting it into your active terminal pane via `send-keys`, so completion is detected instantly via an event-driven IPC hook rather than polling; the command is visible and interactive in your pane. The AI knows your daemon's hostname and whether your pane is SSH'd to a remote machine, and selects the mode accordingly.

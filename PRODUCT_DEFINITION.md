...
- **AI-Powered Capabilities**:
  ...
  - **Passive Pane Monitoring**: AI can use `tmux alert-activity` hooks to actively monitor background panes for output changes asynchronously. If you ask the AI to run a build or wait for a server to spin up, it can set a passive watch and return to you immediately. When the background pane emits text, the daemon interrupts your chat session with an out-of-band `[System]` alert.

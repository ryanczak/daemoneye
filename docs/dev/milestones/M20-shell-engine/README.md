...
| 04 | screen-model ([phase-04-screen-model.md](phase-04-screen-model.md)) | **in-progress** (bounced 2026-09-03, [bug-04-1](bugs/bug-04-1.md)) | `src/shell/screen.rs`: `vt100::Parser` wrapper; `ansi.rs` annotation and `status.rs` classification re-pointed at grid cells; scrollback depth from config. Fixture-driven, no PTY. |

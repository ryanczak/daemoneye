...
| 04 | screen-model ([phase-04-screen-model.md](phase-04-screen-model.md)) | **in-progress** (drafted 2026-09-03) | `src/shell/screen.rs`: `vt100::Parser` wrapper; `ansi.rs` annotation and `status.rs` classification re-pointed at grid cells; scrollback depth from config. Fixture-driven, no PTY. |

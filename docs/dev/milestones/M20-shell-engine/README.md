...
| 02 | pty-marker-protocol ([phase-02-pty-marker-protocol.md](phase-02-pty-marker-protocol.md)) | **in-progress** (bounced 2026-09-03 ×2, [bug-02-1](bugs/bug-02-1.md) fixed; [bug-02-2](bugs/bug-02-2.md), [bug-02-3](bugs/bug-02-3.md) open) | `src/shell/pty.rs`: `portable-pty` spawn, the split-nonce wrapper for bash/zsh/fish, the pure marker parser with the echo-first negative test, exit-code extraction, `\x1f` stripping. One real-PTY test against `bash`. |

...
| 02b | tools-and-default — interactive tool-call approval (raw/cooked-mode coexistence) + tool panels through the ratatui path + code-block-state fix, then flip the `DAEMONEYE_RENDERER` default to ratatui ([phase-02b-tools-and-default.md](phase-02b-tools-and-default.md)) | in-progress |

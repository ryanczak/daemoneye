...
| 08 | split-server — split `daemon/server.rs` (1976) into `server/` : `catchup` (`build_catchup_brief`, `is_valid_pane_id` + tests), `handlers` (15 stateless IPC handlers), `ask` (`handle_ask` orchestrator); `handle_client` stays in `mod.rs` ([phase-08-split-server.md](phase-08-split-server.md)) | review |

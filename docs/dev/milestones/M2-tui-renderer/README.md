...
| 10 | input-editor — visible cursor + word-wrap + multi-line editing + multi-line paste in the ratatui input box ([phase-10-input-editor.md](phase-10-input-editor.md)) | done |
| 11 | interrupt-and-colors — two-press ESC/Ctrl+C interrupt of a streaming agent turn + blood-red border / deep-yellow title recolor of committed command-output panels (`commit_panel`) ([phase-11-interrupt-and-colors.md](phase-11-interrupt-and-colors.md)) | done |
| 12 | split-file-ops — split `daemon/executor/file_ops.rs` (1475) into `file_ops/` : `read` (`run_read_file` + helpers), `write` (`EditArgs`, edit-command builder, response wait), `ops` (`run_edit`/`create`/`delete`/`copy`) ([phase-12-split-file-ops.md](phase-12-split-file-ops.md)) | todo |

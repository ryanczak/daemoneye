...
| 09 | split-config — split `config.rs` (1631) into `config/` : `types` (config structs), `load` (load/resolve/validate/path helpers), `seeds` (seeding fns + embedded asset constants) ([phase-09-split-config.md](phase-09-split-config.md)) | in-progress |

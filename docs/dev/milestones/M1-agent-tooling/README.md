...
| 10 | tmux-surface-and-safe-verbs ([phase-10](phase-10-tmux-surface-and-safe-verbs.md)) — stand-alone tmux-integration phase (was 07c): centralize inline buffer calls into `src/tmux/`, adopt `tmux wait-for` at the one daemon-host-local sentinel-poll site (`read_file` local buffer read); foreground path left untouched | review   |

...
| 10 | docs-and-close ([phase-10-docs-and-close.md](phase-10-docs-and-close.md)) | **done** (approved_first_try, 2026-08-29) | **PE decision 2026-08-29: M18 closes here.** Turn `[sandbox] enabled = true` for real, run background commands through chat against the live runtime, capture start latency / pane readability / sweep behaviour, then the doc sweep (CLAUDE.md, README, `assets/etc/config.toml`) and the retrospective. Escape hatch, egress proxy, `ContainerStatus`, the `log` opcode and staging integration all move to **M19**. |

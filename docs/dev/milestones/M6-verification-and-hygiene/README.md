...
| 03 | [fix-stale-prompt-paths](phase-03-fix-stale-prompt-paths.md) — correct the stale `events.jsonl` references **and the six-path pre-`var/` generation in the knowledge memories** (see defect 5); empty `PENDING_FIX`; fix the two spans the gate cannot see | review      |

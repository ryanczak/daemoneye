...
| 06b | [tmux-control-actions](phase-06b-tmux-control-actions.md) — `split`, `rename_window`, `kill_window` with its daemon-window and chat-window refusals (D5) | in-progress |
...
`done`, 06a is `done` (architect takeover), 06b is drafted (`todo`), 07–08 are
not yet drafted.

...
| 06n | own-teardown-data ([phase-06n-own-teardown-data.md](phase-06n-own-teardown-data.md)) — the two that share the "make the data owned, then wrap" shape: `notify_session` (owned `BgJobInfo`, 2 call sites) + `cleanup_bg_windows` ×2 (additive `BgTeardown` + `run_bg_teardown`, since `SessionEntry` is **not `Clone`**). Whole change compile/clippy/fmt/test-checked at draft time | review      |

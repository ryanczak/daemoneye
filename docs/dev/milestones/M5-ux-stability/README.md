...
| 06r | async-inject-ghost-event ([phase-06r-async-inject-ghost-event.md](phase-06r-async-inject-ghost-event.md)) — make `inject_ghost_event` `async`, `.await` its **12** call sites across 4 files, wrap `notify_chat_panes`. **The last mechanism-B site in the daemon.** Cascade verified non-breaking: an unawaited async call warns rather than errors, so the build stays green and the compiler enumerates the sites | in-progress |

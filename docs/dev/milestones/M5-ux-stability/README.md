...
| 06q | unlock-list-panes ([phase-06q-unlock-list-panes.md](phase-06q-unlock-list-panes.md)) — `handle_list_panes`: `pane_exists` in a synchronous `.filter()` closure **while holding the `cache.panes` RwLock read guard**. Mechanism A **and** B in one site, and the milestone's first phase on a lock other than `SessionStore`. Whole change compile/clippy/fmt/test-checked at draft time | review      |

...
| 06t | bounded-output-session ([phase-06t-bounded-output-session.md](phase-06t-bounded-output-session.md)) — **stage A slice 2**: `src/tmux/session.rs` (9 sites). Five different surrounding shapes, all a **pure substitution** — `bounded_output` returns the same `io::Result<Output>`, so no collapse is needed anywhere | review |
| 06u | stage A slice 3 — `src/tmux/pane.rs` (30 `.output()` sites; needs splitting when drafted). Not drafted | todo |

...
| 06h | tmux-off-runtime-daemon-mod ([phase-06h-tmux-off-runtime-daemon-mod.md](phase-06h-tmux-off-runtime-daemon-mod.md)) — `daemon/mod.rs`: 9 convertible in `run_daemon`; the 6 in `detect_session`/`install_session_hooks` are **sync** (startup path) and are not conversions | review      |

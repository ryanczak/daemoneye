...
| 06f | tmux-off-runtime-executor-tail ([phase-06f-tmux-off-runtime-executor-tail.md](phase-06f-tmux-off-runtime-executor-tail.md)) — `knowledge/pane.rs` (7) + `file_ops/{mod,read}.rs` (5) = 12. Finishes `executor/` | done (approved_first_try) |
...
| 06i | wrap-sync-fns-at-call-sites ([phase-06i-wrap-sync-fns-at-call-sites.md](phase-06i-wrap-sync-fns-at-call-sites.md)) — **slice 1** of the wrap set: `capture_and_archive` (4 call sites) + `gc_bg_windows` (1) = 5 wraps. Establishes wrap-the-caller, which needs no signature or test change | done (approved_first_try) |
| 06m | wrap-sync-fns-slice-2 ([phase-06m-wrap-sync-fns-slice-2.md](phase-06m-wrap-sync-fns-slice-2.md)) — 7 call sites: `detect_session` (1), `install_session_hooks` (2), `get_pane_remote_host` (2 — **a tmux call the scan cannot see**), `notify_chat_panes` (2, N subprocesses per call) | done (approved_first_try) |
...
| 06n | own-teardown-data ([phase-06n-own-teardown-data.md](phase-06n-own-teardown-data.md)) — the two that share the "make the data owned, then wrap" shape: `notify_session` (owned `BgJobInfo`, 2 call sites) + `cleanup_bg_windows` ×2 (additive `BgTeardown` + `run_bg_teardown`, since `SessionEntry` is **not `Clone`**). Whole change compile/clippy/fmt/test-checked at draft time | in-progress |
| 06q | `server/handlers.rs:186` — `pane_exists` in a synchronous `.filter()` closure **while the enclosing block holds a `cache.panes` RwLock read guard**. Two defects in one site; the fix is to drop the guard and rewrite the chain as a loop. Split out of 06n 2026-07-28. Not drafted | todo |
| 06r | `webhook/process.rs:190` — `notify_chat_panes` inside the **synchronous** `inject_ghost_event`, whose 13 call sites across 5 files make this an ordered async cascade rather than a wrap. Split out of 06n 2026-07-28. Not drafted | todo |

...
| 06j | tmux-off-runtime-daemon-tail ([phase-06j-tmux-off-runtime-daemon-tail.md](phase-06j-tmux-off-runtime-daemon-tail.md)) — `gc.rs` (2) + `ghost.rs` (1) + `hook.rs` (1) + `server/ask.rs` (2) = 6. Finishes the daemon's async surface | todo |

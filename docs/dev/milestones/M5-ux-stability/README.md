...
| 06p | wrap set, slice 3 — `close_bg_window` (1) + `watch_pane` (1) in `executor/mod.rs`. Both return a model-visible string, so timeout text is a product decision; `watch_pane` also calls `tokio::spawn` internally, which must be checked against `spawn_blocking`. Not drafted | todo |
| 06n | the four needing a shape change first — `notify_session` (needs an owned `BgJobInfo`; `BgJobInfo<'_>` is not `'static`), `handlers.rs:186` (sync `.filter()` closure in an iterator chain), `process.rs:190` (its caller `inject_ghost_event` is sync, so the wrap moves up the chain), `cleanup_bg_windows` ×2 (`SessionEntry` is **not `Clone`**, so `&self` cannot cross `spawn_blocking`). Not drafted | todo |

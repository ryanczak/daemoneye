...
| 09 | consolidate-loop-ctx ([phase-09-consolidate-loop-ctx.md](phase-09-consolidate-loop-ctx.md)) — `ConversationLoopCtx` + `AskRequest`/`AskContext` for the two high-arity orchestration fns (`run_conversation_loop`, `handle_ask`); resolves the final 2 `TODO(M2)` markers | maint | todo |

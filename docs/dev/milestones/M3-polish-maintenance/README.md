...
| 04 | error-message-quality ([phase-04-error-message-quality.md](phase-04-error-message-quality.md)) — kill the `render_error` `{:?}` debug-dump leak via `Response::kind()`; standardize the three slash-command empty-state messages | ux | review |

...
fn resize_input_area(
    ...
    draw_status_bar(height, width, sb);
}
...
fn collapse_input_area(
    ...
    resize_input_area(height, width, input_rows, 1, start_time, sb);
}
        ...
        tokio::select! {
            _ = sigwinch.recv() => {
                ...
                *chat_height = terminal_height();
                ...
                draw_status_bar(*chat_height, *chat_width, sb);
                render_input_multiline(&state.current, *chat_height, *chat_width, input_rows);
            }
            key = read_key(stdin) => {
                let Some(key) = key else {
                    collapse_input_area(*chat_height, *chat_width, input_rows, start_time, sb);
                    return Ok(None);
                };
                match key {
                    Key::Enter => {
